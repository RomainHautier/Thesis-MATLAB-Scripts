...
for s = 1:length(sol_cases)
    ...
    for i = 1:length(speed)
        ...
        [pcx, pcy, pcz] = wakefilamentplot(2, ['C:\Users\hauti\heli-fwmrtr2\MyCases2rotFW\' num2str(turns) 'turns\d' num2str(speed(i)) '\FWGEOM.DAT'],turns);
        ...
        medscale(1,1:2,i) = median(median(CLs));
        avgV1(1,1:2,i) = mean(vscaled);
        medscale(1,3,i) = abs(medscale(1,2,i)/medscale(1,1,i));
        ...
        for j = 1:length(VBZPSImatout(1,1,:))
            ...
            surf(X,Y,Z,CLs(:,:,j),'EdgeColor','none','FaceAlpha',0.85)
            shading interp;
            % Set the colormap
            ...
            axis equal;
            ax                      = gca;     % gca = get current axes, and store this information in ax
            ax.FontSize             = 22;      % set the property 'FontSize' to 14
