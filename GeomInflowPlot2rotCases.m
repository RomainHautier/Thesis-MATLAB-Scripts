...
for s = 1:length(sol_cases)
    ...
    for i = 1:length(speed)
        ...
        for j = 1:length(VBZPSImatout(1,1,:))
            ...
            ax.YAxis.Exponent       = 0;       % force the y-axis exponent
            ax.TickLabelInterpreter = 'latex'; %
            xl = xlabel('$\frac{x}{R_{MR}}$','Interpreter','latex','FontSize',40);
            xlimit = ceil(max(max(pcx(:,:,2)))*radr);
            ...
            zl = zlabel('$\frac{z}{R_{MR}}$','Interpreter','latex','FontSize',40);
            ...
            legend('Main Rotor','', 'Tail Rotor', 'Interpreter', 'latex','Fontsize',28,'Location','northwest')
