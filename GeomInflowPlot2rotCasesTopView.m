...
for s = 1:length(sol_cases)
    ...
    for i = 1:length(speed)
        ...
        for j = 1:length(fil_index)
            ...
            plot3(pcx(fil_index(j),1:nzt,2)*radr+xdisp(i),pcy(fil_index(j),1:nzt,2)*radr,pcz(fil_index(j),1:nzt,2)*radr,'Color',[0.8500 0.3250 0.0980],'LineWidth',1.2)
            hold on;
            scatter3(pcx(fil_index(j),1:nzt,2)*radr+xdisp(i),pcy(fil_index(j),1:nzt,2)*radr,pcz(fil_index(j),1:nzt,2)*radr,1,[0.8500 0.3250 0.0980],'*')
        end
        ...
        for j = 1:length(VBZPSImatout(1,1,:))
            ...
            % end
            if j == 2
                surf(X+xdisp(i),Y,Z,V,'EdgeColor','none','FaceAlpha',0.85)
                shading interp;
            end
            % Set the colormap
            ...
            yl = ylabel('$\frac{y}{R_{MR}}$','Interpreter','latex','FontSize',40);
            ...
            % end
            % axis equal    
            title([num2str(speed(i)) ' kts'],'Interpreter','latex','FontSize',40)
            legend('Main Rotor','', 'Tail Rotor', 'Interpreter', 'latex','Fontsize',22,'Location','northwest')
